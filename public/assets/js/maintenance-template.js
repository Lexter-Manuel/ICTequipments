...
function createTemplateCard(t) {
    ...
    if (typeNames.length > 0) {
        typeBadges = typeNames.map(name => `<span class="badge bg-light border me-1" style="font-size:10px;">${name}</span>`).join('');
    } else {
        typeBadges = '<span class="badge bg-light border" style="font-size:10px;">No type assigned</span>';
    }
...
<div class="d-flex justify-content-between align-items-start mb-3">
                ...
                <div class="dropdown" onclick="event.stopPropagation()">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li>
                            <button class="dropdown-item text-edit" type="button" onclick="editTemplate(${t.templateId})">
                                <i class="fas fa-edit me-2"></i> Edit
                            </button>
                        </li>
                        <li><hr class="dropdown-divider"></li>
                        <li>
                            <button class="dropdown-item text-danger" type="button" onclick="deleteTemplate(${t.templateId})">
    ...
    newSection.innerHTML = `
        <div class="section-header">
            ...
            <div class="builder-controls">
                <button class="btn btn-xs btn-link p-0 me-2" style="color: var(--text-dark);" onclick="addItem(this)"><i class="fas fa-plus"></i> Add Item</button>
...
function renderPreview(t, container) {
    ...
    container.innerHTML = `
        <div class="builder-paper preview-paper mx-auto p-4" style="max-width: 850px; font-family: 'Times New Roman', serif;">
            
            <div class="text-center mb-4">
                <h5 class="fw-bold text-uppercase mb-1" style="letter-spacing: 1px;">ICT PREVENTIVE MAINTENANCE</h5>
                <p class="text-uppercase fw-bold m-0 preview-subtitle" style="font-size: 14px;">Procedure Checklist</p>
            </div>

            <div class="row g-3 mb-4 small text-uppercase fw-bold">
                <div class="col-6">
                    <div class="d-flex align-items-end mb-2">
                        <span style="width: 130px;">Division/Unit:</span>
                        <div class="flex-grow-1 border-bottom preview-underline"></div>
                    </div>
                    <div class="d-flex align-items-end mb-2">
                ...
                <div class="col-6">
                    ...
                    <div class="d-flex align-items-end mb-2">
                        ...
                        <div class="flex-grow-1 border-bottom preview-underline ps-2">${(t.targetTypeNames || []).join(', ') || formatTypeIds(t.targetTypeId)}</div>
                    </div>
                    <div class="d-flex align-items-end">
            ...
            <table class="table table-bordered table-sm w-100 mb-4 preview-table" style="font-size: 13px;">
            ...
            <div class="preview-remarks-box p-2 mb-4" style="min-height: 80px;">
                <strong class="d-block small text-uppercase mb-2">Remarks / Recommendations:</strong>
                <div class="border-bottom preview-underline mt-4 mx-2"></div>
                <div class="border-bottom preview-underline mt-4 mx-2 mb-2"></div>
            </div>
            ...
            <div class="row pt-4">
                <div class="col-4">
                    ...
                    <div class="text-center">
                        ...
                        <div class="border-top preview-underline mt-1 pt-1 mx-2"></div>
                        <small class="text-uppercase preview-subtitle" style="font-size: 11px;">ICT Staff</small>
                    </div>
                </div>
                <div class="col-4">
                    <small class="d-block mb-4">Checked by:</small>
                    <div class="text-center">
                        <div class="fw-bold text-uppercase" style="color: var(--primary-green);">${sigs.verifiedByName || '&nbsp;'}</div>
                        <div class="border-top preview-underline mt-1 pt-1 mx-2"></div>
                        <small class="text-uppercase" style="font-size: 11px; color: var(--primary-green);">${sigs.verifiedByTitle}</small>
                    </div>
                </div>
                <div class="col-4">
                    <small class="d-block mb-4">Noted by:</small>
                    <div class="text-center">
                        <div class="fw-bold text-uppercase" style="color: var(--primary-green);">${sigs.notedByName || '&nbsp;'}</div>
                        <div class="border-top preview-underline mt-1 pt-1 mx-2"></div>
                        <small class="text-uppercase" style="font-size: 11px; color: var(--primary-green);">${sigs.notedByTitle}</small>
                    </div>
                </div>
            </div>
...
function saveTemplate() {
    ...
    if (selectedTypes.length === 0) {
        Alerts.warning('Please select at least one equipment type.');
        return;
    }
...
function editTemplate(id) {
    ...
    fetch(`../ajax/manage_templates.php?action=get&id=${id}`)
        .then(r => r.json())
        .then(data => {
            if(!data.success) { Alerts.error(data.message); return; }
...
function deleteTemplate(id) {
    Alerts.confirmDelete('this template', function() {
        fetch(`${BASE_URL}ajax/manage_templates.php?action=delete&id=${id}`)
            .then(r => r.json())
            .then(data => {
                if (data.success) {
                    Alerts.success('Template deleted successfully.');
                    reloadCurrentPage();
                } else {
                    Alerts.error(data.message);
                }
...
function loadBuilderEquipmentTypes() {
    ...
    fetch(`${BASE_URL}ajax/get_equipment_types.php?scope=all`)
        .then(r => r.json())
        .then(res => {
            if (res.success) {
...
function renderTypeCheckboxes(selectedIds) {
    ...
    // Map typeId to icon
    var typeIcons = {
        '1': 'fa-desktop',      // System Unit
        '2': 'fa-desktop',      // All-in-One
        '3': 'fa-tv',           // Monitor
        '4': 'fa-print',        // Printer
        '5': 'fa-laptop',       // Laptop
        '6': 'fa-computer-mouse',// Mouse
        '7': 'fa-keyboard',     // Keyboard
        '8': 'fa-video',        // CCTV
        '9': 'fa-server'        // NAS
    };

    var html = '<div class="type-picker-grid">';
    equipmentTypeCache.forEach(function(type) {
        ...
        var icon = typeIcons[String(type.typeId)] || 'fa-cogs';
        html += `
            <label class="type-chip${activeClass}" for="typeChk_${type.typeId}">
                <input type="checkbox" value="${type.typeId}" id="typeChk_${type.typeId}" ${checked} onchange="onTypeChipChange(this)" hidden>
                <i class="fas ${icon} type-chip-icon"></i>
                <span class="type-chip-name">${type.typeName}</span>
                <span class="type-chip-check"><i class="fas fa-check"></i></span>
            </label>`;
    });
    html += '</div>';

    html += `<div class="type-picker-actions">
        <button class="type-picker-action-btn" onclick="selectAllTypes(true)"><i class="fas fa-check-double me-1"></i>Select All</button>
        <button class="type-picker-action-btn" onclick="selectAllTypes(false)"><i class="fas fa-times me-1"></i>Clear</button>
    </div>`;
...
function onTypeChipChange(cb) {
    var label = cb.closest('.type-chip');
    if (cb.checked) {
...
function closeTypePickerOutside(e) {
    var wrapper = document.getElementById('typeMultiSelectDropdown');
    if (!wrapper.contains(e.target)) {
        document.getElementById('typeMultiSelectMenu').classList.remove('show');
